...
export default async req => {
  ...
  if (!supabaseUrl || !serviceKey || !clientId || !secret) {
    return new Response('Strava server configuration is incomplete.', { status: 500 });
  }
  ...
  const current = await currentUser(supabaseUrl, serviceKey, accessToken);
