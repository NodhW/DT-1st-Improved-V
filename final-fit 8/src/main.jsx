...
function SocialTab({ groupsList, chatList, onOpenGroup, onOpenFriend, onCreateGroup, onJoinGroup, onLeaveGroup }) {
      ...
      <section className="setting">
        ...
        {sources.map(source => (
          <div className="connection" key={source.id}>
            ...
            <button
              disabled={syncingCalendarId === source.id}
              onClick={async () => {
                setSyncingCalendarId(source.id);
                setMsg('Syncing calendar…');
                try {
                  ...
                  const recurring = Number(response.recurringEventsImportedAsFirstOccurrence || 0);
                  setMsg(`Calendar synced: ${imported} imported, ${scanned} scanned${recurring ? ` · ${recurring} recurring event${recurring === 1 ? '' : 's'} imported as first occurrence` : ''}.`);
                  await refreshConnections();
                  await onSaved();
                } catch (error) {
                  setMsg(`Calendar sync failed: ${errorText(error)}`);
                } finally {
                  setSyncingCalendarId(null);
                }
              }}
            >{syncingCalendarId === source.id ? 'Syncing…' : 'Sync'}</button>
