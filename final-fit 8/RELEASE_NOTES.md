...
- Calendar sync now uses the Supabase publishable key only to verify the signed-in user token and a server-only Supabase secret key for privileged calendar/schedule writes.
- `SUPABASE_SECRET_KEY` is preferred; legacy `SUPABASE_SERVICE_ROLE_KEY` remains supported as a fallback.
- `SUPABASE_PUBLISHABLE_KEY` is preferred; legacy `SUPABASE_ANON_KEY` remains supported as a fallback.
- The client shows `Syncing…` while a calendar import is running and surfaces structured server errors.
- The calendar function validates redirects, rejects private/local targets, enforces a 5 MB response cap and 12-second timeout, verifies that the response is an iCalendar feed, and reports imported/scanned/failed counts.
