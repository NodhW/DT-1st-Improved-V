  ...
  chat: async ({ message, context = {} }) => {
    ...
    const response = await fetch('/api/gemini/chat', {
      ...
      body: JSON.stringify({
        ...
        context: { user_id: current.id, ...context }
      })
    });
    let payload = null;
    try { payload = await response.json(); } catch { payload = null; }
    if (!response.ok) throw new Error(payload?.error || 'Fit AI is unavailable right now.');
...
export const fitness = {
  ...
  startWorkout: async ({ title = 'Workout', planWorkoutId = null } = {}) => {
    await user();
    return result(supabase.rpc('start_workout', { p_title: clean(title), p_plan_workout_id: planWorkoutId }));
  },
  addExercise: async ({ workoutId, exerciseId, position = 0 }) => {
    await user();
    return result(supabase.rpc('add_workout_exercise', { p_workout_id: workoutId, p_exercise_id: exerciseId, p_position: position }));
  },
  addSet: async ({ workoutExerciseId, setNumber, reps = null, weight = null, durationSeconds = null, rpe = null }) => {
    await user();
    return result(supabase.rpc('add_workout_set', { p_workout_exercise_id: workoutExerciseId, p_set_number: setNumber, p_reps: reps, p_weight: weight, p_duration_seconds: durationSeconds, p_rpe: rpe }));
  },
  deleteSet: async setId => { await user(); return result(supabase.rpc('delete_workout_set', { p_set_id: setId })); },
  deleteExercise: async workoutExerciseId => { await user(); return result(supabase.rpc('delete_workout_exercise', { p_workout_exercise_id: workoutExerciseId })); },
  cancelWorkout: async workoutId => { await user(); return result(supabase.rpc('cancel_workout', { p_workout_id: workoutId })); },
  completeWorkout: async workoutId => result(supabase.rpc('complete_workout', { p_workout_id: workoutId })),
  history: async (limit = 20) => {
    await user();
    return result(supabase.rpc('get_my_workout_history', { p_limit: Math.max(1, Math.min(100, Number(limit) || 20)) }));
  },
  records: async () => {
    const current=await user();
