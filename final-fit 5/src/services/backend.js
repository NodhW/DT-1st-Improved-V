...
const PROFILE_CARD_COLUMNS = 'id,display_name,avatar_url,friend_code,university,university_verified';
...
export const profile = {
  ...
  byId: id => result(supabase.from('profiles').select(PROFILE_CARD_COLUMNS).eq('id', id).single()),
...
async function profilesByIds(ids) {
  ...
  const rows = await result(supabase.from('profiles').select(PROFILE_CARD_COLUMNS).in('id', unique));
...
export const friends = {
  byCode: code => result(
    supabase
      ...
      .eq('friend_code', normalizeCode(code))
      .single()
  ),
  requests: async () => {
    const current = await user();
