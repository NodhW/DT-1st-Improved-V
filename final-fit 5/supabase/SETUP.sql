...
as $$
declare
  activity_day date := (p_occurred_at at time zone 'UTC')::date;
  pts integer := 0;
  inserted_id uuid;
  last_date date;
begin
  if p_source='self_report' then
    ...
    pts := case when activity_day >= current_date-1 and activity_day <= current_date then 50 else 0 end;
  else
  ...
  if activity_day=current_date and last_date is distinct from current_date then
...
set search_path=public,pg_temp
as $$
declare g public.groups; c public.conversations;
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  ...
  insert into public.group_members(group_id,user_id) values(g.id,auth.uid()) on conflict(group_id,user_id) do nothing;
  delete from public.group_streak_days where group_id=g.id and activity_date=current_date
    and not exists(select 1 from public.activity_log a where a.user_id=auth.uid() and a.activity_date=current_date);
...
begin
  ...
  join public.activity_log a on a.user_id=gm.user_id and a.activity_date=current_date
  where gm.group_id=p_group_id;
  select exists(select 1 from public.activity_log where user_id=auth.uid() and activity_date=current_date) into my_done;
