...
import './styles.css';
import { findBestSlot } from './lib/scheduling.js';
      ...
      if (/streak/i.test(textValue) && groupId) {
        ...
        try {
          const aiText = await ai.chat({
            message: textValue,
            context: {
              display_name: me?.display_name || 'Member',
              university: me?.university || null,
              university_verified: Boolean(me?.university_verified),
              points_balance: Number(me?.points_balance || 0),
              current_streak: activeStreak(me),
              selected_group: groupName() || null,
              today_schedule: scheduleItems.filter(event => isToday(event.starts_at)).map(event => ({ title: event.title, starts_at: event.starts_at, ends_at: event.ends_at, event_type: event.event_type }))
            }
          });
          if (aiText) addAiMessage({ text: aiText });
          else throw new Error('No AI response returned.');
        } catch {
...
function SocialTab({ groupsList, chatList, friendsList, onOpenGroup, onOpenFriend, onCreateGroup, onJoinGroup, onLeaveGroup, onFriends, feed, reloadFeed, pendingCount, onAddFriend }) {
  ...
  return (
    <div className="page">
      <div className="subtabs">
        ...
        <button className={sub === 'feed' ? 'active' : ''} onClick={() => setSub('feed')}><MessageSquare />Community</button>
      </div>
      ...
      {sub === 'groups' ? (
        ...
        <FeedTab items={feed || []} reload={reloadFeed} pendingCount={pendingCount || 0} onAddFriend={onAddFriend} />
      )}
    </div>
  );
}
...
function ChallengeCard({ challenge, me, onJoin }) {
      ...
      {mine ? (
        <>
          ...
          <div className="challenge-actions">
            ...
            {!mine.completed && <small className="muted-text">Progress updates from logged activity</small>}
          </div>
        </>
    ...
    <div className="rewards-panel">
      ...
      {sub === 'challenges' ? (
        <>
          ...
          {list.map(challenge => <ChallengeCard key={challenge.id} challenge={challenge} me={me} onJoin={join} />)}
  ...
  return (
    <div className="app">
      ...
      <main>
        ...
        {tab === 'challenges' && <RewardsTab me={me} groupsList={groupsList} reload={load} />}
      </main>
      ...
      <nav>
        ...
        <button className={`badged ${tab === 'challenges' ? 'active' : ''}`} onClick={() => setTab('challenges')}><Trophy /><span>Challenges</span></button>
      </nav>
