...
begin
  ...
  set university=trim(p_university),
      university_verification_requested_at=case when university_verified then university_verification_requested_at else now() end,
      updated_at=now()
  where id=auth.uid()
  returning * into p;
  return p;
...
begin
  if auth.uid() is null then raise exception 'Not authenticated'; end if;
  if upper(trim(coalesce(p_friend_code,''))) !~ '^(?:[A-Z0-9]{6}|[A-Z0-9]{4}-[A-Z0-9]{4})$' then return; end if;
  return query
...
begin
  ...
  -- Serialize both directions so simultaneous A→B/B→A calls cannot create duplicate rows.
  ...
  -- Reuse the exact-direction row first.
  select * into desired from public.friendships
  where requester_id=auth.uid() and receiver_id=p_receiver_id
  order by created_at desc limit 1;
  if desired.id is not null then
    if desired.status in ('accepted','pending') then return desired; end if;
    if desired.status='declined' then
      update public.friendships set status='pending',created_at=now()
      where id=desired.id returning * into r;
      return r;
    end if;
  end if;

  -- An accepted/pending reverse request is already a live relationship.
  select * into existing from public.friendships
  where requester_id=p_receiver_id and receiver_id=auth.uid()
    and status in ('accepted','pending')
  order by created_at desc limit 1;
  if existing.id is not null then return existing; end if;

  -- Reuse a reverse declined row by flipping its direction, avoiding the directional unique index.
  select * into existing from public.friendships
  where requester_id=p_receiver_id and receiver_id=auth.uid() and status='declined'
  order by created_at desc limit 1;
  if existing.id is not null then
    update public.friendships
    set requester_id=auth.uid(),receiver_id=p_receiver_id,status='pending',created_at=now()
...
('Plank','core',array['core'],array[]::text[],'brace','Maintain a straight line and steady breathing.'),
('Push-Up','strength',array['chest','triceps','shoulders','core'],array[]::text[],'push','Keep the body rigid and lower under control.'),
('Running','cardio',array['cardio','legs'],array[]::text[],'locomotion','Run at the selected intensity.'),
...
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
declare w public.workouts; total numeric:=0; sets integer:=0; avgrpe numeric; s record; est numeric; oldpr numeric; completed_count integer:=0; set_count integer:=0; achievement_id uuid; new_pr boolean:=false; first_completion boolean:=false; begin
